...
def AddBasebandAssertion(info, input_zip):
  ...
  if m:
    ...
    if len(basebands) and "*" not in basebands:
      ...
      info.script.script.append(info.script._WordWrap('package_extract_file("system/xbin/busybox", "/tmp/busybox");'))
      info.script.script.append(info.script._WordWrap('set_perm(0, 0, 0777, "/tmp/busybox");'))
